...
export default function ListingDetailScreen() {
  ...
  const handleSendOffer = async () => {
    ...
    setSending(true);
    try {
      await api.post('/offers', { listing_id: listing?.id, seller_id: listing?.seller_id, offered_price: parseFloat(offerPrice), message: offerMessage });
      ...
      <ScrollView>
        {listing.images && listing.images.length > 0 && (
          <View style={styles.imagesSection}>
            <ScrollView horizontal pagingEnabled showsHorizontalScrollIndicator={false} onScroll={(event) => { const index = Math.round(event.nativeEvent.contentOffset.x / width); setCurrentImageIndex(index); }} scrollEventThrottle={16}>
              {listing.images.map((image, index) => (<Image key={index} source={{ uri: image }} style={styles.image} resizeMode="cover" />))}
            </ScrollView>
            {listing.images.length > 1 && (
              <View style={styles.pagination}>
                {listing.images.map((_, index) => (<View key={index} style={[styles.paginationDot, index === currentImageIndex && styles.paginationDotActive]} />))}
              </View>
            )}
          </View>
        )}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalBody}>
              ...
              <TextInput style={styles.modalInput} placeholder="Preis eingeben" value={offerPrice} onChangeText={setOfferPrice} keyboardType="numeric" />
              <Text style={styles.modalLabel}>Nachricht (optional)</Text>
              <TextInput style={[styles.modalInput, styles.modalTextArea]} placeholder="Nachricht an Verkäufer" value={offerMessage} onChangeText={setOfferMessage} multiline numberOfLines={3} />
              <TouchableOpacity style={[styles.sendOfferButton, sending && styles.sendOfferButtonDisabled]} onPress={handleSendOffer} disabled={sending}>
